<template>
  <q-header class="bg-transparent text-dark">
    <div class="justify-end flex q-pa-sm items-center">
      <q-icon
        name="eva-moon-outline"
        size="18px"
        :color="$q.dark.mode ? 'grey' : 'dark'"
      ></q-icon>
      <q-toggle
        v-model="SettingStore.dark_theme"
        val="1"
        @update:model-value="setDarkmode"
      />
    </div>
  </q-header>
  <q-page padding class="flex flex-center">
    <div>
      <q-card flat :class="{ 'bg-grey600': $q.dark.mode }">
        <q-card-section>
          <q-form @submit="onSubmit" @reset="onReset">
            <div class="flex justify-center q-mb-lg">
              <svg
                version="1.0"
                xmlns="http://www.w3.org/2000/svg"
                width="60"
                height="60"
                viewBox="0 0 1280.000000 1201.000000"
                preserveAspectRatio="xMidYMid meet"
              >
                <g
                  transform="translate(0.000000,1201.000000) scale(0.100000,-0.100000)"
                  fill="#14b8a6"
                  stroke="none"
                >
                  <path
                    d="M302 11547 c-22 -23 -22 -26 -22 -430 0 -224 3 -407 8 -407 4 0 68 -7 142 -15 411 -46 705 -124 865 -230 135 -90 210 -249 262 -555 17 -99 18 -315 21 -3795 2 -2662 0 -3730 -8 -3835 -31 -400 -120 -616 -300 -725 -160 -97 -447 -171 -822 -212 l-168 -18 0 -404 0 -405 27 -28 27 -28 2641 0 2641 0 27 28 27 28 0 405 0 405 -132 13 c-166 16 -384 52 -517 86 -409 103 -540 235 -611 614 -38 204 -40 302 -40 1736 l1 1400 401 362 c221 200 402 362 403 360 30 -35 2742 -3753 2772 -3799 185 -286 204 -531 49 -633 -100 -67 -370 -108 -853 -131 l-143 -7 0 -405 0 -406 27 -28 27 -28 2722 0 2722 0 26 31 27 31 -3 396 -3 397 -110 37 c-234 80 -502 234 -740 424 -130 104 -407 378 -524 519 -117 140 -162 200 -2156 2904 l-1787 2423 28 28 c88 84 2275 2100 2359 2174 418 368 937 660 1410 790 200 55 346 77 548 83 171 6 180 7 208 30 l29 25 0 383 c0 372 -1 383 -21 409 l-20 26 -2472 0 -2471 0 -28 -27 -28 -27 0 -376 0 -376 33 -32 33 -32 169 0 c354 0 669 -57 850 -153 83 -45 169 -127 202 -195 25 -51 28 -67 27 -152 -1 -75 -7 -113 -29 -180 -55 -169 -170 -359 -335 -554 -63 -73 -601 -590 -1649 -1584 -856 -811 -1589 -1507 -1629 -1546 l-73 -71 4 1770 c3 1612 5 1779 20 1875 49 301 127 465 262 555 160 106 454 184 865 230 74 8 138 15 143 15 4 0 7 184 7 408 l0 409 -23 21 -23 22 -2650 0 -2651 0 -21 -23z"
                  />
                </g>
              </svg>
            </div>

            <q-input
              outlined
              v-model="username"
              :label="$t('Username')"
              :bg-color="$q.dark.mode ? 'grey300' : 'secondary'"
              no-error-icon
              lazy-rules
              :rules="[
                (val) => (val && val.length > 0) || this.$t('This is required'),
              ]"
              autocorrect="off"
              autocapitalize="off"
              autocomplete="off"
              spellcheck="false"
            >
              <template v-slot:prepend>
                <q-icon name="eva-person-outline" />
              </template>
            </q-input>

            <PasswordField
              v-model:password="password"
              :defaul_value="password"
            ></PasswordField>

            <q-btn
              color="primary"
              :label="$t('Login')"
              class="fit radius-10"
              unelevated
              :size="$q.screen.gt.sm ? 'xl' : 'lg'"
              type="submit"
              no-caps
              :loading="loading"
            />

            <div class="flex justify-center q-mt-md">
              <q-btn
                flat
                :label="$t('Forgot Password') + '?'"
                no-caps
                :color="$q.dark.mode ? 'grey300' : 'accent'"
                class="text-weight-light"
                to="/forgot-password"
              ></q-btn>
            </div>
          </q-form>
        </q-card-section>
      </q-card>
    </div>
  </q-page>
</template>

<script>
import { defineAsyncComponent } from "vue";
import APIinterface from "src/api/APIinterface";
import { useIdentityStore } from "stores/IdentityStore";
import { useSettingStore } from "stores/SettingStore";

export default {
  name: "LoginPage",
  components: {
    PasswordField: defineAsyncComponent(() =>
      import("components/PasswordField.vue")
    ),
  },
  data() {
    return {
      username: "",
      password: "",
      loading: false,
    };
  },
  setup() {
    const IdentityStore = useIdentityStore();
    const SettingStore = useSettingStore();
    return {
      IdentityStore,
      SettingStore,
    };
  },
  mounted() {},
  methods: {
    setDarkmode(value) {
      this.$q.dark.set(value);
    },
    onSubmit() {
      this.loading = true;
      APIinterface.fetchData(
        "Login",
        "username=" + this.username + "&password=" + this.password
      )
        .then((data) => {
          this.IdentityStore.auth_token = data.details.user_token;
          this.IdentityStore.user_data = data.details.user_data;
          this.$router.replace("/current");
        })
        .catch((error) => {
          APIinterface.notify(
            this.$q,
            this.$t("Error"),
            error,
            "myerror",
            "highlight_off",
            "bottom"
          );
        })
        .then((data) => {
          this.loading = false;
        });
    },
  },
};
</script>
