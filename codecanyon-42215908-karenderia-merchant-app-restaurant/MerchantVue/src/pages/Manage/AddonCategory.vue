              ...
              <q-item-section avatar top>
                ...
                >
                  <template v-slot:loading>
                    <q-skeleton square class="bg-grey-2 fit" />
                  </template>
                </q-img>
              </q-item-section>
              <q-item-section>
                ...
                <q-item-label caption lines="2">
                  {{ items.description }}</q-item-label
                >
                <q-item-label>
                  <q-toggle
                    color="primary"
                ...
                <div class="flex q-gutter-x-md">
                  <q-btn
                    ...
                    :to="{
                      ...
                      query: { subcat_id: items.id },
                    }"
                  ></q-btn>

                  <q-btn
                    v-if="AccessStore.hasAccess('food.addoncategory_delete')"
...
export default {
  ...
  mounted() {
    this.MenuStore.page_title = this.$t("Addon Category List");

    if (this.DataStore.dataList?.addoncategory_data) {
      this.data = this.DataStore.dataList?.addoncategory_data?.data;
      this.page = this.DataStore.dataList?.addoncategory_data?.page;
      this.hasMore = this.DataStore.dataList?.addoncategory_data?.hasMore;
    } else {
      setTimeout(() => {
  ...
  beforeUnmount() {
    this.DataStore.dataList.addoncategory_data = {
      data: this.data,
      page: this.page,
  ...
  methods: {
    ...
    async afterConfirm() {
      try {
        ...
        await APIinterface.fetchDataByTokenPost("deleteAddonCategory", params);
