...
export default {
  ...
  mounted() {
    this.MenuStore.page_title = this.$t("Size List");

    if (this.DataStore.dataList?.sizelist) {
      this.data = this.DataStore.dataList?.sizelist?.data;
      this.page = this.DataStore.dataList?.sizelist?.page;
      this.hasMore = this.DataStore.dataList?.sizelist?.hasMore;
    } else {
      setTimeout(() => {
  ...
  beforeUnmount() {
    this.DataStore.dataList.sizelist = {
      data: this.data,
      page: this.page,
  ...
  methods: {
    ...
    async fetchCategory(index, done) {
      try {
        ...
        const response = await APIinterface.fetchGet(`getSizeList?${params}`);
        this.page++;
