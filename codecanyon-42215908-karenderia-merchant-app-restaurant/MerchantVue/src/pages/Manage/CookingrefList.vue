...
export default {
  ...
  mounted() {
    this.MenuStore.page_title = this.$t("Cooking Reference");

    if (this.DataStore.dataList?.cookingref_list) {
      this.data = this.DataStore.dataList?.cookingref_list?.data;
      this.page = this.DataStore.dataList?.cookingref_list?.page;
      this.hasMore = this.DataStore.dataList?.cookingref_list?.hasMore;
    } else {
      setTimeout(() => {
  ...
  beforeUnmount() {
    this.DataStore.dataList.cookingref_list = {
      data: this.data,
      page: this.page,
  ...
  methods: {
    ...
    async fetchCategory(index, done) {
      try {
        ...
        const response = await APIinterface.fetchGet(`CookingList?${params}`);
        this.page++;
