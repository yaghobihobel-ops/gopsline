...
export default {
  ...
  mounted() {
    this.MenuStore.page_title = this.$t("Category");

    if (this.DataStore.dataList?.category_data) {
      this.data = this.DataStore.dataList?.category_data?.data;
      this.page = this.DataStore.dataList?.category_data?.page;
      this.hasMore = this.DataStore.dataList?.category_data?.hasMore;
    } else {
      setTimeout(() => {
  ...
  beforeUnmount() {
    this.DataStore.dataList.category_data = {
      data: this.data,
      page: this.page,
  ...
  methods: {
    ...
    async fetchCategory(index, done) {
      try {
        ...
        const response = await APIinterface.fetchGet(`fetchCategory?${params}`);
