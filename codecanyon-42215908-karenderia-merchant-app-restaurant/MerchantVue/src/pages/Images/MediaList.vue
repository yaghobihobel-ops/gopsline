...
export default {
  ...
  mounted() {
    this.DataStore.page_title = this.$t("Media");

    if (this.DataStore.dataList?.media_list) {
      this.data = this.DataStore.dataList?.media_list?.data;
      this.page = this.DataStore.dataList?.media_list?.page;
      this.hasMore = this.DataStore.dataList?.media_list?.hasMore;
    } else {
      setTimeout(() => {
  ...
  beforeUnmount() {
    this.DataStore.dataList.media_list = {
      data: this.data,
      page: this.page,
  ...
  methods: {
    async fetchData(index, done) {
      try {
        ...
        const response = await APIinterface.fetchGet(`mediaList?${params}`);
        this.page++;
