...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      APIinterface.fetchDataByTokenPost("getGallery", "id=" + this.id)
        .then((data) => {
    ...
    formSubmit() {
      ...
      APIinterface.fetchDataByTokenPost(
        !APIinterface.empty(this.id) ? "UpdateGallery" : "AddGallery",
        params
      )
        .then((data) => {
          APIinterface.ShowSuccessful(data.msg, this.$q);
          this.DataStore.cleanData("gallery_list");
          this.$router.replace({
            path: "/images/gallery",
          });
        })
