...
export default {
  ...
  mounted() {
    this.DataStore.page_title = this.$t("Gallery");

    if (this.DataStore.dataList?.gallery_list) {
      this.data = this.DataStore.dataList?.gallery_list?.data;
      this.page = this.DataStore.dataList?.gallery_list?.page;
      this.hasMore = this.DataStore.dataList?.gallery_list?.hasMore;
    } else {
      setTimeout(() => {
  ...
  beforeUnmount() {
    this.DataStore.dataList.gallery_list = {
      data: this.data,
      page: this.page,
  ...
  methods: {
    async fetchData(index, done) {
      try {
        ...
        const response = await APIinterface.fetchGet(`galleryList?${params}`);
        this.page++;
    ...
    async afterConfirm() {
      try {
        ...
        await APIinterface.fetchDataByTokenPost("deleteGallery", params);
