  ...
  <q-pull-to-refresh @refresh="refresh">
    ...
    >
      <q-toolbar>
        ...
        ></q-btn>
        <q-toolbar-title>
          <template v-if="isEdit">{{ $t("Edit Addon Category") }}</template>
          <template v-else>{{ $t("Add Addon Category") }}</template>
        </q-toolbar-title>

        <template v-if="AccessStore.hasAccess('food.addoncategory_delete')">
          <q-btn
    ...
    >
      ...
      <q-form v-else @submit="onSubmit">
        <div class="q-gutter-md">
          ...
          <q-input
            outlined
            v-model="subcategory_description"
            :label="$t('Description')"
            stack-label
            color="grey-5"
            autogrow
...
export default {
  ...
  created() {
    this.subcat_id = this.$route.query.subcat_id;
    if (this.subcat_id > 0) {
      this.getCategory();
    }
  },
  methods: {
    ...
    getCategory(done) {
      ...
      APIinterface.fetchDataByTokenPost(
        ...
        .then((data) => {
          this.subcategory_name = data.details.subcategory_name;
          this.subcategory_description = data.details.subcategory_description;
    ...
    onSubmit() {
      this.loading = true;
      APIinterface.fetchDataByToken("addAddonCategory", {
        subcat_id: this.subcat_id,
        ...
        .then((data) => {
          ...
          this.DataStore.cleanData("addoncategory_data");
          if (!this.subcat_id) {
            this.$router.replace("/manage/addoncategory");
          }
        })
        .catch((error) => {
