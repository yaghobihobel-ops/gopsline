      ...
      <q-list>
        ...
        >
          <q-card flat class="myshadow bg-white q-mb-md">
            <q-item>
              <q-item-section>
                <q-item-label>{{ items.offer_name }}</q-item-label>
...
export default {
  ...
  mounted() {
    this.DataStore.page_title = this.$t("Offers");

    if (this.DataStore.dataList?.offer_list) {
      this.data = this.DataStore.dataList?.offer_list?.data;
      this.page = this.DataStore.dataList?.offer_list?.page;
      this.hasMore = this.DataStore.dataList?.offer_list?.hasMore;
    } else {
      setTimeout(() => {
  ...
  beforeUnmount() {
    this.DataStore.dataList.offer_list = {
      data: this.data,
      page: this.page,
  ...
  methods: {
    async fetchData(index, done) {
      try {
        ...
        const response = await APIinterface.fetchGet(`offerList?${params}`);
        this.page++;
    ...
    async afterConfirm() {
      try {
        ...
        await APIinterface.fetchDataByTokenPost("deleteOffers", params);
