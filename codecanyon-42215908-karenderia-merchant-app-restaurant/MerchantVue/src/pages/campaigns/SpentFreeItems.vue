              ...
              <q-item-section side top>
                <q-item-label>
                  <q-badge
                    ...
                    :text-color="OrderStore.itemStatus[items.status_raw]?.text"
                    rounded
                ...
                <q-item-label>
                  <div class="flex q-gutter-x-md">
                    <q-btn
                      ...
                      size="sm"
                      :to="{
                        path: '/campaigns/update_free_items',
                        query: { id: items.id },
                      }"
                    ></q-btn>
                    <q-btn
...
export default {
  ...
  mounted() {
    this.DataStore.page_title = this.$t("Spend-Based Free Item");

    if (this.DataStore.dataList?.freeItemList) {
      this.data = this.DataStore.dataList?.freeItemList?.data;
      this.page = this.DataStore.dataList?.freeItemList?.page;
      this.hasMore = this.DataStore.dataList?.freeItemList?.hasMore;
    } else {
      setTimeout(() => {
  ...
  beforeUnmount() {
    this.DataStore.dataList.freeItemList = {
      data: this.data,
      page: this.page,
  ...
  methods: {
    async fetchFromApi(index, done) {
      try {
        ...
        const response = await APIinterface.fetchGet(`freeItemList?${params}`);
        this.page++;
    ...
    async afterConfirm() {
      try {
        ...
        await APIinterface.fetchDataByTokenPost("deleteFreeItems", params);
