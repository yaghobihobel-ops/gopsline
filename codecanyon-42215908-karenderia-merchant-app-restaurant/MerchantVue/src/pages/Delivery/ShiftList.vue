              ...
              <template v-for="item in items" :key="item">
                <q-item clickable>
                  ...
                  <q-item-section top>
                    ...
                    <q-item-label>
                      {{ $t("Slot") }} - {{ item.max_allow_slot }}</q-item-label
                    >
                  </q-item-section>
                  <q-item-section side class="row items-stretch" top>
                    <div class="column items-center col-12 q-gutter-y-md">
                      ...
                      <div class="col">
                        <q-btn
                          v-if="
                            AccessStore.hasAccess('merchantdriver.shift_delete')
                          "
                          round
                          ...
                          unelevated
                          @click.stop="
                            deleteItems(item.shift_uuid, items, index)
                          "
                        >
                        </q-btn>
                      </div>
...
export default {
  ...
  methods: {
    ...
    deleteRecords(id, data, index) {
      ...
      APIinterface.fetchDataByTokenPost("deleteShiftSchedule", "id=" + id)
        .then((response) => {
          data.splice(index, 1);
        })
        .catch((error) => {
    ...
    List(index, done) {
      this.loading = true;
      APIinterface.fetchDataByTokenPost("ShiftList", "&page=" + index)
        .then((data) => {
          if (data.code == 1) {
            this.page = index;
