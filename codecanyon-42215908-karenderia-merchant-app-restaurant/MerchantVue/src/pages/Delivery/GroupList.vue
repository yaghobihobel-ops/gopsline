                    ...
                    <div class="column items-center col-12 q-gutter-y-md">
                      <div class="col">
                        <q-btn
                          ...
                          unelevated
                          :to="{
                            path: '/driver/update-group',
                            query: { id: item.group_uuid },
                          }"
                        />
                      </div>
                      <div class="col">
                        <q-btn
                          v-if="
                            AccessStore.hasAccess('merchantdriver.group_delete')
                          "
                          round
                          ...
                          unelevated
                          @click.stop="
                            deleteItems(item.group_uuid, items, index)
                          "
                        >
                        </q-btn>
                      </div>
...
export default {
  ...
  methods: {
    ...
    deleteRecords(id, data, index) {
      ...
      APIinterface.fetchDataByTokenPost("deleteGroup", "id=" + id)
        .then((response) => {
          data.splice(index, 1);
        })
        .catch((error) => {})
    ...
    List(index, done) {
      this.loading = true;
      APIinterface.fetchDataByTokenPost("GroupList", "&page=" + index)
        .then((data) => {
          if (data.code == 1) {
            this.page = index;
