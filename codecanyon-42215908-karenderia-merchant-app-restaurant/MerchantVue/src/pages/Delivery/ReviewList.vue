              ...
              <template v-for="item in items" :key="item">
                <q-item clickable>
                  ...
                  <q-item-section top>
                    ...
                    <q-item-label>
                      <q-badge color="green">{{ item.rating }}</q-badge>
                    </q-item-label>
                  </q-item-section>
...
export default {
  ...
  methods: {
    ...
    List(index, done) {
      this.loading = true;
      APIinterface.fetchDataByTokenPost("ReviewList", "&page=" + index)
        .then((data) => {
          if (data.code == 1) {
            this.page = index;
