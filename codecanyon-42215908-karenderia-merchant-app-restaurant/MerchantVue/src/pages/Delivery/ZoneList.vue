                    ...
                    <div class="column items-center col-12 q-gutter-y-md">
                      <div class="col">
                        <q-btn
                          ...
                          unelevated
                          :to="{
                            path: '/driver/update-zone',
                            query: { id: item.zone_uuid },
                          }"
                        />
                      </div>
                      <div class="col">
                        <q-btn
                          v-if="
                            AccessStore.hasAccess('merchantdriver.zone_delete')
                          "
                          round
                          ...
                          unelevated
                          @click.stop="
                            deleteItems(item.zone_uuid, items, index)
                          "
                        >
                        </q-btn>
                      </div>
...
export default {
  ...
  methods: {
    ...
    deleteRecords(id, data, index) {
      ...
      APIinterface.fetchDataByTokenPost("deleteZones", "id=" + id)
        .then((response) => {
          data.splice(index, 1);
        })
        .catch((error) => {
    ...
    List(index, done) {
      this.loading = true;
      APIinterface.fetchDataByTokenPost("ZoneList", "&page=" + index)
        .then((data) => {
          if (data.code == 1) {
            this.page = index;
