                    ...
                    <div class="column items-center col-12 q-gutter-y-md">
                      <div class="col">
                        <q-btn
                          ...
                          unelevated
                          :to="{
                            path: '/driver/update-car',
                            query: { id: item.vehicle_uuid },
                          }"
                        />
                      </div>
                      <div class="col">
                        <q-btn
                          v-if="
                            AccessStore.hasAccess('merchantdriver.delete_car')
                          "
                          round
                          ...
                          unelevated
                          @click.stop="
                            deleteItems(item.vehicle_uuid, items, index)
                          "
                        >
                        </q-btn>
                      </div>
...
export default {
  ...
  methods: {
    ...
    deleteRecords(id, data, index) {
      ...
      APIinterface.fetchDataByTokenPost("deleteCar", "id=" + id)
        .then((response) => {
          data.splice(index, 1);
        })
        .catch((error) => {})
    ...
    List(index, done) {
      this.loading = true;
      APIinterface.fetchDataByTokenPost("carList", "&page=" + index)
        .then((data) => {
          if (data.code == 1) {
            this.page = index;
