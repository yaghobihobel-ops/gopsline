              ...
              <template v-for="item in items" :key="item">
                <q-item clickable>
                  ...
                  <q-item-section top>
                    ...
                    <q-item-label class="text-grey-1" v-if="item.reference_id">
                      {{ item.reference_id }}
                    </q-item-label>
                    <q-item-label class="text-darkx">
                    ...
                    <div class="column items-center col-12 q-gutter-y-md">
                      <div class="col">
                        <q-btn
                          ...
                          size="sm"
                          unelevated
                          @click="getDetails(item.collection_uuid)"
                        />
                      </div>
                      <div class="col">
                        <q-btn
                          v-if="
                            AccessStore.hasAccess(
                              'merchantdriver.collect_cash_delete'
                            )
                          "
                          round
                          ...
                          unelevated
                          @click.stop="
                            deleteItems(item.collection_uuid, items, index)
                          "
                        >
                        </q-btn>
                      </div>
...
export default {
  ...
  components: {
    ...
    CollectCashDetails: defineAsyncComponent(() =>
      import("components/CollectCashDetails.vue")
    ),
  },
  setup() {
  ...
  methods: {
    ...
    List(index, done) {
      this.loading = true;
      APIinterface.fetchDataByTokenPost("collectCashList", "&page=" + index)
        .then((data) => {
          if (data.code == 1) {
            this.page = index;
    ...
    deleteRecords(id, data, index) {
      ...
      APIinterface.fetchDataByTokenPost("deleteCollectCash", "id=" + id)
        .then((response) => {
          data.splice(index, 1);
        })
        .catch((error) => {})
