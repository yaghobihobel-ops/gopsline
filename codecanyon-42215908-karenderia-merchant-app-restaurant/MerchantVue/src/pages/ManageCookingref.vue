  ...
  <q-pull-to-refresh @refresh="refresh">
    <q-header class="myshadow bg-white text-dark">
      <q-toolbar>
        ...
        ></q-btn>
        <q-toolbar-title>
          <template v-if="isEdit">{{ $t("Edit Cooking Reference") }}</template>
          <template v-else>{{ $t("Add Cooking Reference") }}</template>
        </q-toolbar-title>
        <q-btn
...
export default {
  ...
  methods: {
    ...
    getData(done) {
      ...
      APIinterface.fetchDataByTokenPost("getCooking", "id=" + this.id)
        .then((data) => {
    ...
    onSubmit() {
      this.loading = true;
      APIinterface.fetchDataByToken("addCooking", {
        id: this.id,
        name: this.name,
