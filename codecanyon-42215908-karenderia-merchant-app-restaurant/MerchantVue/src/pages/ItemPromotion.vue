      ...
      <template v-else>
        <template v-if="hasData">
          <div class="row items-center justify-between">
            <div class="text-weight-bold">{{ $t("Promo") }}</div>

            <template v-if="AccessStore.hasAccess('food.itempromo_delete')">
              <q-btn
                v-if="hasData2"
...
export default {
  ...
  methods: {
    refresh(done) {
      this.getSalePromotion(done);
    },
    getSalePromotion(done) {
      this.loading = true;
      APIinterface.fetchDataByTokenPost("getSalePromotion", "id=" + this.id)
        .then((data) => {
    ...
    afterConfirm() {
      ...
      APIinterface.fetchDataByTokenPost(
        "deletePromotion",
        "item_uuid=" + this.id + "&promo_id=" + this.data_to_delete.promo_id
      )
        .then((data) => {
