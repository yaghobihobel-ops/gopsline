<template>
  <q-pull-to-refresh @refresh="refresh">
    ...
    >
              ...
              <template v-for="item in items" :key="item">
                <q-item clickable>
                  ...
                  <q-item-section top>
                    ...
                    <q-item-label>
                      {{ $t("Order#") }} {{ item.transaction_ref_id }}

                      <q-badge
                        :color="
                          DataStore.bank_status_color[item.status_raw]
                            ? DataStore.bank_status_color[item.status_raw]
                            : 'amber'
                        "
                      >
                    ...
                    <q-item-label caption>
                      {{ item.amount }}
                      <template v-if="item.reference_number">
                        - {{ item.reference_number }}</template
                      >
                    </q-item-label>
                  </q-item-section>
                  <q-item-section side class="row items-stretch" top>
                    <div class="column items-center col-12 q-gutter-y-md">
                      <div class="col">
                        <q-btn
                          round
                          :color="$q.dark.mode ? 'grey600' : 'green'"
                          :text-color="$q.dark.mode ? 'grey300' : 'white'"
                          icon="las la-eye"
                          size="sm"
                          unelevated
                          @click.stop="viewImage(item.proof_image)"
                        />
                      </div>
                      <div class="col">
                        <q-btn
                          ...
                          unelevated
                          :to="{
                            path: '/payments/update-bank-deposit',
                            query: { id: item.deposit_uuid },
                          }"
                        />
                      </div>
                      <div class="col">
                        <q-btn
                          ...
                          unelevated
                          @click.stop="
                            deleteItems(item.deposit_uuid, items, index)
                          "
                        >
                        </q-btn>
                      </div>
      ...
      >
        <q-card>
          <q-card-section style="max-height: 60vh" class="q-pa-none">
            <q-space />
            <q-btn icon="close" flat round dense v-close-popup />
            <div class="row items-start">
              <q-img :src="imageURL" :ratio="1" fit="fill">
...
export default {
  ...
  methods: {
    ...
    deleteRecords(id, data, index) {
      ...
      APIinterface.fetchDataByTokenPost("deleteBankDeposit", "id=" + id)
        .then((response) => {
          data.splice(index, 1);
        })
        .catch((error) => {
    ...
    List(index, done) {
      this.loading = true;
      APIinterface.fetchDataByTokenPost("bankdepositlist", "&page=" + index)
        .then((data) => {
          if (data.code == 1) {
            this.page = index;
