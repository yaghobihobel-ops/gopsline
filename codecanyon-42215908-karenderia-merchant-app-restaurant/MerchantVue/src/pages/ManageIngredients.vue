  ...
  <q-pull-to-refresh @refresh="refresh">
    <q-header class="bg-white text-dark myshadow">
      <q-toolbar>
        ...
        ></q-btn>
        <q-toolbar-title>
          <template v-if="isEdit">{{ $t("Edit Ingredients") }}</template>
          <template v-else>{{ $t("Add Ingredients") }}</template>
        </q-toolbar-title>
        <q-btn
...
export default {
  ...
  methods: {
    ...
    getData(done) {
      ...
      APIinterface.fetchDataByTokenPost("getIngredients", "id=" + this.id)
        .then((data) => {
    ...
    onSubmit() {
      this.loading = true;
      APIinterface.fetchDataByToken("addIngredients", {
        id: this.id,
        name: this.name,
        ...
        .then((data) => {
          ...
          this.DataStore.cleanData("ingredient_list");
          if (!this.id) {
            this.$router.push("/manage/ingredients");
          }
        })
        .catch((error) => {
          APIinterface.ShowAlert(error, this.$q.capacitor, this.$q);
