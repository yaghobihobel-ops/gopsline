<template>
  ...
  >
    <q-toolbar>
      ...
      <template v-if="AccessStore.hasAccess('booking.room_delete')">
        <q-btn
          v-if="hasData"
          @click="confirmDelete"
          flat
...
export default {
  ...
  methods: {
    ...
    deleteRecords() {
      ...
      APIinterface.fetchDataByTokenPost("deleteRooms", "id=" + this.id)
        .then((response) => {
          this.$router.replace({
            path: "/table/rooms",
          });
    ...
    formSubmit() {
      ...
      APIinterface.fetchDataByTokenPost(
        !APIinterface.empty(this.id) ? "UpdateRoom" : "CreateRoom",
        params
      )
        .then((data) => {
    ...
    getRoom() {
      this.loading = true;
      APIinterface.fetchDataByTokenPost("getRoom", "id=" + this.id)
        .then((data) => {
          this.room_name = data.details.room_name;
