<template>
  <q-pull-to-refresh @refresh="refresh">
    ...
    >
      ...
      <q-infinite-scroll ref="nscroll" @load="tableShift" :offset="250">
                ...
                <q-item>
                  ...
                  <q-item-section side class="row items-stretch" top>
                    <div class="column items-center col-12 q-gutter-y-md">
                      ...
                      <div class="col">
                        <q-btn
                          v-if="AccessStore.hasAccess('booking.shift_delete')"
                          round
                          ...
                          size="sm"
                          unelevated
                          @click="deleteItems(item.shift_uuid, items, index)"
                        >
                        </q-btn>
                      </div>
                    </div>
...
export default {
  ...
  methods: {
    ...
    deleteRecords(id, data, index) {
      ...
      APIinterface.fetchDataByTokenPost("deleteShift", "id=" + id)
        .then((response) => {
          data.splice(index, 1);
        })
        .catch((error) => {})
    ...
    tableShift(index, done) {
      this.loading = true;
      APIinterface.fetchDataByTokenPost("tableShift", "&page=" + index)
        .then((data) => {
          if (data.code == 1) {
            this.page = index;
